...
```{r}
# change format of all missing values so they're all recognized as NA
# rename columns in data set using the pipe operator (add using command+shift+m)
toolik_weather <- read_csv(here::here("data", "data-raw", "toolik_weather.csv"), na = "#N/A") %>% 
# use clean_names under the janitor packages, default is lower snake case
  janitor::clean_names()
...
```{r}
myAwesomeChart <- ggplot(data = toolik_weather, aes(x = month, y = daily_air_temp_mean_c)) +
  geom_jitter(aes(color = daily_air_temp_mean_c)) +
  scale_color_gradient(low = 'yellow2', high = 'tomato1')
  labs (x = "Month",
        y = "Daily Mean Air Temperature (Celcius)",
        title = "Toolik Station Daily Air Temperature")
myAwesomeChart
  
#save chart, adding file name using here::here
# automatically saves the chart/info right before ggsave() code 
